...
export function useCart() {
   ...
   */
  const addToCart = (product: ProductDto, variant?: ProductVariantDto, quantity: number = 1) => {
   ...
   */
  const updateItemQuantity = (productId: string, quantity: number, variantId?: string) => {
  ...
  const isInCart = (productId: string, variantId?: string): boolean => {
    return items.some((item) => item.product.id === productId && item.variant?.id === variantId)
  }
  ...
  const getCartItem = (productId: string, variantId?: string): CartItem | undefined => {
    return items.find((item) => item.product.id === productId && item.variant?.id === variantId)
  }
