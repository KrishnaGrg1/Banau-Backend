...
function RouteComponent() {
  ...
  const { tenantConfig: data } = useLoaderData({ from: '__root__' })
  const { data: product, isLoading } = usePublicProductBySlug({
    subdomain: String(data?.existingTenant.subdomain),
    slug,
  })
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 space-y-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            <span className="text-3xl font-bold text-foreground">Rs.{price.toFixed(2)}</span>
            {compareAt && (
          ...
          <div className="flex items-center gap-2">
            <span className={`h-2 w-2 rounded-full ${inStock ? 'bg-emerald-500' : 'bg-red-400'}`} />
            <span
          ...
          {hasVariants && (
            <div className="space-y-3 border-t border-border pt-6">
                ...
                {product.variants!.map((variant) => (
                  ...
                  >
                    ...
                    {variant.option1Value && (
                      <span className="ml-1 text-muted-foreground">· {variant.option1Value}</span>
                    )}
              ...
              {hasVariants && !selectedVariant && (
                <p className="text-xs text-amber-600">Please select a variant to add to cart</p>
              )}
            </div>
          )}
      ...
      <section className="border-t border-border pt-16 space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-foreground">More Products</h2>
          <Link
            to="/products"
