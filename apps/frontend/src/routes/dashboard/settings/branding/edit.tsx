...
export const Route = createFileRoute('/dashboard/settings/branding/edit')({
...
function EditBrandingPage() {
  ...
  const { mutateAsync, isSuccess, isError, error, isPending } =
    useUpdateTenantSetting()
  const { data: settingsData } = useTenantSetting()
  ...
  // Sync existing settings into form after async load
  useEffect(() => {
    ...
    form.setFieldValue('backgroundColorCode', settingsData.backgroundColorCode)
    form.setFieldValue(
    ...
    <div className="flex flex-col flex-1 gap-8 p-6 max-w-4xl">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Update your store's visual identity — logo, favicon, colors, and
          landing page copy.
        </p>
      </div>
            ...
            <form.Field name="logo">
              {(field) => (
                <AssetUpload
                  ...
                  existingLabel={
                    assetsData?.logo?.fileName
                      ? `Current: ${assetsData.logo.fileName}`
                      : undefined
                  }
                  onClear={() => {
            ...
            <form.Field name="favicon">
              {(field) => (
                <AssetUpload
                  ...
                  existingLabel={
                    assetsData?.favicon?.fileName
                      ? `Current: ${assetsData.favicon.fileName}`
                      : undefined
                  }
                  onClear={() => {
                    field.handleChange(null)
