...
function RouteComponent() {
  ...
  const storeInfo = [
    { label: 'Store Name', value: tenantConfig?.existingTenant?.name },
    { label: 'Subdomain', value: tenantConfig?.existingTenant?.subdomain, mono: true },
    { label: 'Contact', value: tenantConfig?.existingTenant?.email },
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 space-y-24">
      ...
      <section className="text-center space-y-6 max-w-2xl mx-auto">
        ...
        <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-foreground">
          We are <span className="text-primary">{tenantConfig?.existingTenant?.name}</span>
        </h1>
        <p className="text-muted-foreground leading-relaxed text-base">
          Welcome to {tenantConfig?.existingTenant?.name}. We're passionate about bringing you the
          best products with a seamless shopping experience. Built on Banau — a modern platform for
          stores that mean business.
        </p>
        ...
        <div className="space-y-8">
          <div className="space-y-4">
            ...
            <p className="text-muted-foreground leading-relaxed">
              {tenantConfig?.existingTenant?.name} was founded with a simple belief — great products
              should be easy to find and even easier to buy. We've curated everything in our
              catalogue with care, quality, and our customers in mind.
            </p>
            <p className="text-muted-foreground leading-relaxed">
              Powered by Banau, our store is built for speed, reliability, and a modern shopping
          ...
          <div className="space-y-3">
            {tenantConfig?.existingTenant?.email && (
              <a
                href={`mailto:${tenantConfig.existingTenant.email}`}
            ...
            <div className="flex items-center gap-3 text-sm text-muted-foreground">
              ...
              <span className="font-mono text-xs">
                {tenantConfig?.existingTenant?.subdomain}.banau.com
              </span>
            </div>
          </div>
        </div>
      ...
      <section className="rounded-3xl border border-border bg-card p-12 text-center space-y-6">
        <h2 className="text-2xl font-bold tracking-tight text-foreground">Ready to shop?</h2>
